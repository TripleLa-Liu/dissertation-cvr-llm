"""
Ali-CCP LLM Encoder V2 — RLMRec-style alignment (keep ID + regularize with text)
====================================================================================
...
RESULTS_DIR = r"E:\BaiduNetdiskDownload\Dataset\_processed\v2_results"
...
# independent cache (avoids the v1/v1_full dict-schema mismatch bug hit 2026-07-21)
ITEM_EMBED_CACHE = os.path.join(RESULTS_DIR, "item_llm_embeddings.pkl")
CHECKPOINT_PATH = os.path.join(RESULTS_DIR, "esmm_v2_aligned.pt")
METRICS_PATH = os.path.join(RESULTS_DIR, "esmm_v2_aligned_metrics.json")
...
def train_epoch(model, loader, optimizer, bce, lambda_align, temperature):
    ...
    for u, item_idx, item_llm_row, click, purchase in loader:
        ...
        if len(unique_idx) > 1:
            # map each unique train-vocab idx to its llm row via the batch's
            # own item_idx/item_llm_row pairing (build a lookup on the fly)
            idx_to_row = {}
...
def main():
    ...
    print("\n" + "=" * 70)
    print("FINAL RESULTS (V2 — RLMRec-style ID + LLM alignment)")
    print("=" * 70)
