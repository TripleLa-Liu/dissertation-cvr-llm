"""
Ali-CCP LLM Encoder V1 — frozen small-LM item embeddings (run locally)
=========================================================================
...
VOCAB_PATH = os.path.join(RESULTS_DIR, "id_vocab_v1.pkl")
CHECKPOINT_PATH = os.path.join(RESULTS_DIR, "esmm_llm_v1.pt")
METRICS_PATH = os.path.join(RESULTS_DIR, "esmm_llm_v1_metrics.json")

LM_NAME = "all-MiniLM-L6-v2"   # small, fast sentence embedding model (384-dim)
LM_EMBED_DIM = 384
USER_EMBED_DIM = 32
ADAPTER_DIM = 32                # LLM embedding projected down to this, matching user_emb dim
HIDDEN = [128, 64]
BATCH_SIZE = 4096
EPOCHS = 15
PATIENCE = 3
...
def build_item_embeddings():
    ...
    # Append an explicit UNK row (zeros) at the end. In principle every item
    # in our splits has pseudo-text (extract_item_pseudo_text.py guarantees
    # this), so this row should never actually be looked up — but mapping
    # missing lookups to -1 is unsafe (undefined/wrap-around indexing
    # behaviour in nn.Embedding, caught via a synthetic-data test 2026-07-21),
    # so we always have a real, safe index to fall back to instead.
...
def encode_item_rows(series, item_id_to_row, unk_row):
    """Map item_id -> row index into the frozen embedding matrix. Every item
    in our splits has pseudo-text (extract_item_pseudo_text.py guarantees
...
class ESMM_LLM_V1(nn.Module):
    def __init__(self, n_users, item_embed_matrix, user_dim=32, adapter_dim=32,
        ...
        # frozen item LLM embedding table (not updated during training)
        ...
        # trainable adapter: projects frozen LM embedding down to adapter_dim
        # (small MLP, akin to UniSRec's whitening+adaptor idea, simplified)
...
# ------------------------------------------------------------------
# Train / eval (identical logic to baseline)
# ------------------------------------------------------------------
...
def main():
    ...
    # only train user_emb + towers + adapter — item_emb_frozen has no grad by construction
    ...
    print("\n" + "=" * 70)
    print("FINAL RESULTS (LLM Encoder V1 — frozen MiniLM item text)")
    print("=" * 70)
