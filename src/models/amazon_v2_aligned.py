"""
Amazon Reviews'23 (Video_Games) V2 — RLMRec-style ID + LLM alignment
============================================================================
Dissertation: LLM-Enhanced Dynamic Graph Networks for CVR Prediction
Author: Liu Yize | UCL MSc KIDS

Amazon-side counterpart to llm_encoder_v2_aligned.py (2026-08-04 supervisor
request to fill out the model-variant x embedder x dataset comparison
matrix — previously Amazon only had the ID baseline (amazon_id_baseline.py)
and the REPLACE-pattern text embedding (amazon_text_embedding.py), with no
ALIGN-pattern (V2) counterpart to compare against.

Architecture is llm_encoder_v2_aligned.py's ESMM_V2_Aligned pattern,
adapted to Amazon's single binary-label task (same adaptation
amazon_id_baseline.py / amazon_text_embedding.py already made from the
Ali-CCP two-stage CTR/CVR baseline/V1): one BCE loss against `label`
(interaction yes/no), not a CTR+CTCVR pair.

  - user_emb, item_emb: learned nn.Embedding, identical to
    amazon_id_baseline.py (index 0 = UNK, only exercised by cold-start
    items at test time).
  - item_llm_proj: trainable MLP, frozen MiniLM item-text embedding -> dim
    (same frozen encoder as amazon_text_embedding.py, for a fair
    model-for-model comparison).
  - Per-row item representation: item_emb(idx) if idx != 0 (seen in
    train), else item_llm_proj(frozen_llm_emb) (cold-start item) — same
    masked blend as llm_encoder_v2_aligned.py.
  - Loss = BCE(label) + lambda_align * contrastive_loss, same symmetric
    InfoNCE alignment term as the Ali-CCP version.
...
RESULTS_DIR = os.path.join(WORK_DIR, "amazon", "v2_aligned_results")
...
# independent cache — separate from text_embedding_results/ (V1-equivalent)
# so both can be compared side by side without either overwriting the other
ITEM_EMBED_CACHE = os.path.join(RESULTS_DIR, "item_llm_embeddings.pkl")
VOCAB_PATH = os.path.join(RESULTS_DIR, "id_vocab.pkl")
CHECKPOINT_PATH = os.path.join(RESULTS_DIR, "amazon_v2_aligned.pt")
METRICS_PATH = os.path.join(RESULTS_DIR, "amazon_v2_aligned_metrics.json")

LM_NAME = "all-MiniLM-L6-v2"   # same encoder as amazon_text_embedding.py, for a fair comparison
USER_EMBED_DIM = 32
ITEM_EMBED_DIM = 32
HIDDEN = [128, 64]
BATCH_SIZE = 1024
...
def parse_args():
    ...
    the original unsuffixed checkpoint/metrics filenames so
    amazon_v3_hybrid.py's hardcoded paths keep working without any change."""
...
# ------------------------------------------------------------------
# Frozen item LLM embeddings (same pattern as amazon_text_embedding.py,
...
# ------------------------------------------------------------------
# Model (single-task version of ESMM_V2_Aligned)
# ------------------------------------------------------------------
...
def main():
    ...
    print("\n" + "=" * 70)
    print("FINAL RESULTS (Amazon Video_Games — V2 RLMRec-style ID + LLM alignment)")
    print("=" * 70)
    ...
    print(f"\nCompare against amazon_baseline_results/amazon_id_baseline_metrics.json (ID) "
          f"and text_embedding_results/amazon_text_embedding_metrics.json (V1-equivalent REPLACE).")
