"""
Ali-CCP LLM Encoder V1-Full-MPNet — item + user text replace, MPNet encoder
====================================================================================
Dissertation: LLM-Enhanced Dynamic Graph Networks for CVR Prediction
Author: Liu Yize | UCL MSc KIDS

"Different Embedders" experiment, extended to V1-Full (2026-08-04
supervisor request for a more robust comparison matrix across model
variant x embedder x dataset). Identical to llm_encoder_v1_full.py, with
the frozen sentence-transformer swapped from all-MiniLM-L6-v2 (384-dim) to
all-mpnet-base-v2 (768-dim). Completes the encoder-swap grid alongside
llm_encoder_v1_mpnet.py and the existing llm_encoder_v2_mpnet.py, so
"does a bigger encoder help" can be answered for all three LLM-integrated
variants, not just V2.

Everything else (architecture, train/val/test split, evaluation protocol)
is unchanged from llm_encoder_v1_full.py — see that file for full design
rationale. Writes to a separate v1_full_mpnet_results/ directory so this
run doesn't overwrite the MiniLM-based V1-Full checkpoint.
...
RESULTS_DIR = r"D:\Study\migration_package\processed_data\v1_full_mpnet_results"
...
# independent cache — separate from v1_full_results/ (MiniLM) so both can
...
CHECKPOINT_PATH = os.path.join(RESULTS_DIR, "esmm_llm_v1_full_mpnet.pt")
METRICS_PATH = os.path.join(RESULTS_DIR, "esmm_llm_v1_full_mpnet_metrics.json")

LM_NAME = "all-mpnet-base-v2"   # only change vs. llm_encoder_v1_full.py (was all-MiniLM-L6-v2)
ADAPTER_DIM = 32
HIDDEN = [128, 64]
BATCH_SIZE = 4096
EPOCHS = 15
...
# ------------------------------------------------------------------
# Model — both sides frozen LLM embeddings + adapters (identical to
# ESMM_LLM_V1_Full in llm_encoder_v1_full.py — dims are read dynamically
# from the embedding matrices, so this class needs no changes for the
# larger 768-dim MPNet vectors)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Train / eval (same logic as llm_encoder_v1_full.py)
# ------------------------------------------------------------------
...
def main():
    ...
    print("\n" + "=" * 70)
    print("FINAL RESULTS (LLM Encoder V1-Full-MPNet — item + user frozen MPNet text)")
    print("=" * 70)
    ...
    print(f"\nCompare against v1_full_results/esmm_llm_v1_full_metrics.json (MiniLM) "
